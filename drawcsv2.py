...
if os.path.isfile(piblocksFile):
    with open(piblocksFile, 'r') as csvfile:
...
with open(build_file, 'r') as csvfile:
    data = csv.reader(csvfile, delimiter=',', quotechar='|')
    for row in data:
        ...
        for house_block in row:
            ...
            # crashes
                ...
                if extra > 0:
                    ...
                    block_type = 0
                    # Can we draw this block ? Pi Minecraft only has a limited
                    # set of blocks
                if (block_number in validBlocks):
                    # We can draw the block, but is it a special block that
                    # needs to be drawn later?
                    if (block_number in specialList):
                        ...
                        # Our special block list looks a bit like this
                        # ['64,33,1,17,1','64,33,2,17,2'...]
