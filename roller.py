...
def checkBlockIsHit(starting_block):
    ...
    if (block_hits):
        for blockHit in block_hits:
            ...
            if (bx == starting_block.x and by ==
                    starting_block.y and bz == starting_block.z):
...
def drawCoaster(dx, current_y, starting_block):
    ...
    for angle in range(0, 180 + steps, steps):
        ...
        # As our sine wave goes up and down, we use fabs() to get the absolute
        # difference
        if (fabs(dy - last_y) > 1):
            # draw from the last height to the new height so there aren't any
            # gaps
            mc.setBlocks(
                dx,
                last_y,
            ...
            mc.setBlocks(
                ...
                block.WOOL.id,
                1)
            mc.setBlocks(dx, dy + 1, roller_z - 2, dx, dy + 1,
                         roller_z - 1, block.WATER_STATIONARY)
        # Move to the right
        dx = dx + 1
