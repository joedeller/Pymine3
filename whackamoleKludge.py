...
class MoleGame():
    ...
    def scanForHits(self):
        ...
        # Python 3 breaks this as the tuple lookup no longer works
        ...
        if (block_hits):
            for blockHit in block_hits:
                ...
                cmp = blockHit.pos
                for actmole in self.activeMoles:
                   # print (str(cmp), ":",str(actmole))
                    if str(cmp) == str(actmole):
                    #if cmp == actmole:
                    #   print ("Match of actMole")
