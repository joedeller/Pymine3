...
class MoleGame():
    ...
    def __init__(self, mc, playerPos, width, depth):
        ...
        self.scoreBoardPos = minecraft.Vec3(
            playerPos.x + 6, playerPos.y + 1, playerPos.z - depth - 4)
    ...
    def showMole(self):
        ...
        if (mole_count < 4):
            ...
            molePos = self.emptyMoles[
                random.randint(
                    0, len(
                        self.emptyMoles) - 1)]
            # Add this mole to the active list and remove it from the empty
            # list
    ...
    def hideMole(self):
        ...
        if (mole_count > 0):
            ...
            # Remember lists start counting from 0, so our random number needs
            # be from 0 to moles - 1
    ...
    def scanForHits(self):
        ...
        # Python 3 broke this as the tuple lookup no longer worked
        # creating a separate tuple seems to fix it
        block_hits = mc.events.pollBlockHits()
        # Were any blocks hit ?
        if (block_hits):
            for blockHit in block_hits:
               # print ("player hit a block", blockHit.pos)
                # Fix for Python 3
                cmp = (blockHit.pos.x,blockHit.pos.y,blockHit.pos.z)
                if cmp in self.activeMoles:
                #if (blockHit.pos in self.activeMoles): <- Old Python 2 code
                    ...
                    self.DrawMessage(
                         self.scoreBoardPos.x, self.scoreBoardPos.y, self.scoreBoardPos.z, str(
                                self.score))
    ...
    #            else:
    #                print (blockHit.pos, " is not in ", self.activeMoles)
    def setupCharacters(self):
        # Create a new dictionary, the space character is all zeros
        # This is a small subset of the font dictionary as we only want the
        # numbers
...
for count in range(5, -1, -1):
    myGame.DrawMessage(
        myGame.scoreBoardPos.x,
        myGame.scoreBoardPos.y,
        myGame.scoreBoardPos.z,
        str(count))
    time.sleep(1)
...
# The shuffling is done in the background and carries on until the time
# limit is up
