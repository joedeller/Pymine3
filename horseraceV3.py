    ...
    while min(horses) >= finish_line:
        for horse in range(0, horseCount):
            ...
            for move in range(0, moves):
                ...
                # Rub out the horse and then move it forward one block at a
                # time
    ...
    while start_race is False:
        ...
        if (block_hits):
            for block_hit in block_hits:
                ...
                if (block_hit.pos.x == x and block_hit.pos.y ==
                        y and block_hit.pos.z == z):
                    print("StartRace!")
                    start_race = True
                else:
                    # The player hit something, but not the right thing so
                    # return false
